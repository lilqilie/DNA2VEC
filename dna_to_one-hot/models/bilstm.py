...
class BiLstmModel(BaseModel):
    def __init__(self, config):
        super(BiLstmModel, self).__init__(config=config)
        ...
        with tf.name_scope("Bi-LSTM"):
            for idx, hidden_size in enumerate(self.config["hidden_sizes"]):
                with tf.name_scope("Bi-LSTM" + str(idx)):
                    ...
                    inputs = tf.constant(value=X, shape=(1778, 1000, 4), dtype=tf.float32)
                    outputs, current_state = tf.nn.bidirectional_dynamic_rnn(cell_fw=lstm_fw_cell, cell_bw=lstm_bw_cell,
