...
from keras.layers import Dropout, Flatten, BatchNormalization, Input, Add, Concatenate, Bidirectional, \
    Softmax, Lambda
...
def bi_lstm_att(config):
    ...
    attention_pre = Dense(64, name='attention_vec')(lstm_out)  # [b_size,maxlen,64]
    attention_probs = Softmax()(attention_pre)  # [b_size,maxlen,64]
    attention_mul = Lambda(lambda x: x[0] * x[1])([attention_probs, lstm_out])
